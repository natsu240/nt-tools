...
BRANCH_SWITCH_CREATE_RE="${BRANCH_CMD_HEAD}git[[:space:]]+switch[[:space:]]+(-[[:alnum:]]*[cC]|--create|--force-create)([[:space:]]|\$)"
# `git checkout` は他の引数が先に来ることがある（`git checkout --track -b foo`）ので、`-b` / `-B` がどの位置にあっても拾う。ただしパイプ・コマンド区切りは越えない。
BRANCH_CHECKOUT_CREATE_RE="${BRANCH_CMD_HEAD}git[[:space:]]+checkout[[:space:]]+([^|;&]*[[:space:]])?-[[:alnum:]]*[bB]([[:space:]]|\$)"
